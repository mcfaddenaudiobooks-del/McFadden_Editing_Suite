...
5. **Synced Caption Writer (v1.0-beta):** Creates a `.srt` captions from a given text, and matches the timeline from a given audiofile. Outputs `.srt` file for captioning word for word from the text. Complete with punctuation, capitalization, spelling etc.
...
```bash 
   McFadden_Audiobooks_Editing_Suite.py
```
